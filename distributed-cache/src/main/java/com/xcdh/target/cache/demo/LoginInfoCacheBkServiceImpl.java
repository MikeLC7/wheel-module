...
@Service("loginInfoCacheBkService")
public class LoginInfoCacheBkServiceImpl  {
    private static final Logger logger = LoggerFactory.getLogger(LoginInfoCacheBkServiceImpl.class);

    @Autowired
    @Qualifier("cacheUtilBk")
    ...
    public Boolean cacheUserLoginInfo(Long userId, Map<String, String> info) {
        ...
        if (!CollectionUtil.isEmpty(info)) {
            ...
            distributedCache.putHashToCache(key, distributedCache.getEmptyMap(), key.getRefreshExpireSeconds());
            distributedCache.logSetRefresh(key);
        }
        return true;
    }
